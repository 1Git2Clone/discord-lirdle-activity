...
Model.prototype = {
  ...
  loadLocalStorage() {
    ...
    if (savedState.date !== currentDate) {
      if (!savedState.finished) {
        doFetch('unfinished', {
          date: currentDate,
          from: getInternalDateNumber(savedState.date),
          count: savedState.guessWords.length,
        });
    ...
    this.view.populateBoardFromSaveableState(
      this.saveableState.numBoardRows,
      this.saveableState.guessWords,
      this.saveableState.scores,
      this.saveableState.markers,
    );
    updateSolver(
      this.saveableState.guessWords,
      this.saveableState.scores,
      this.solverData,
      this.saveableState.finished,
    );
    ...
    if (this.saveableState.finished) {
      ...
      this.view.updateShowNumLeft(
        this.prefs.showNumLeft,
        lastNumLeftIndex,
        this.solverData.possibleWordCounts[lastNumLeftIndex],
      );
    }
    ...
    try {
      ...
      for (const cls of ['show-lie', 'show-perceived-truth']) {
        for (const elt of Array.from(
          this.view.board.querySelectorAll(`div.letter-row div.letter-box.${cls}`),
        )) {
  ...
  initState() {
    ...
    this.lettersByPosition = {
      black: {},
      yellow: {}, // hash of letter to # occurrences (don't deal with duplicates)
      green: ['', '', '', '', ''],
  ...
  async checkGuess() {
    ...
    try {
      ...
      if (guessedIt) {
        ...
        this.view.updateShowNumLeft(
          this.prefs.showNumLeft,
          this.solverData.level - 1,
          this.solverData.possibleWords.length,
        );
        doFetch('guess', {
          date: this.saveableState.date,
          count: this.guessCount,
          numLeft: this.solverData.possibleWords.length,
        });
  ...
  getShareText() {
    ...
    return [
      `Lirdle game #${gameNumber}`,
      '',
      `Solved in ${scores.length} tr${scores.length === 1 ? 'y!' : 'ies.'}`,
      '',
      scoreLines.join('\n'),
      '',
      'lirdle.com - the lying word game',
    ].join('\n');
  },
  ...
  updateShowNumLeftStatus(showNumLeft) {
    ...
    this.savePrefs();
    doFetch('toggleShowNumLeft', {
      date: this.saveableState.date,
      checked: showNumLeft,
      count: this.guessCount || 0,
    });
  },
};
