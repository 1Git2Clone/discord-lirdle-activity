...
export async function generateGridDashboard(players, title) {
	...
	for (let i = 0; i < N; i++) {
		...
		ctx.textAlign = 'center';
		const dName = player.username.length > 12 ? player.username.substring(0, 10) + '..' : player.username;
		...
		for (let row = 0; row < 6; row++) {
			...
			if (showPill && row === 0) {
				...
				createRoundRectPath(ctx, gridX, cellY + ((M_CELL - M_BLOCK) / 2), GRID_WIDTH, M_BLOCK, M_RAD * 2);
				ctx.fill();
				ctx.fillStyle = '#ffffff';
