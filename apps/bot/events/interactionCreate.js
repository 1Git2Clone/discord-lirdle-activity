...
export default async (client, interaction) => {
	try {
		...
		if (interaction.isChatInputCommand()) {
			...
			fileMatch = interactionFiles.find(file =>
				path.basename(file) === `${fileBase}.js`
			);
		} else {
		...
		if (!fileMatch) {
			clog(console.error, `[apps/bot/events/interactionCreate.js] The interaction file was not found for interaction: ${interaction.customId || interaction.commandName}`);
			return;
		}
